...
public class SelectorAdapter extends BaseAdapter implements Filterable {
    ...
    public SelectorAdapter(Context _context, ArrayList<Pair> _arrayListPair, int typeOfWeek) {
        this.context = _context;
        this.arrayListPair = _arrayListPair;
        mValue = new ArrayList<>();
        for (int i = 0; i < arrayListPair.size(); i++) {
            if (arrayListPair.get(i).week_type == typeOfWeek || arrayListPair.get(i).week_type == 2){
